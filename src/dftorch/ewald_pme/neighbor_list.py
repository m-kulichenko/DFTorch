        ...
        for s_x in range(-1, min(cx_dim - 1, 2)):
            for s_y in range(-1, min(cy_dim - 1, 2)):
                for s_z in range(-1, min(cz_dim - 1, 2)):
                    ...
                    cell_flat_idx = (
                        (nx * cy_dim * cz_dim) + (ny * cz_dim) + nz
                    ) * num_candids

                    # Load candidate indices
                    candid_ids = tl.load(
                        cell_list_ptr + cell_flat_idx + candid_offsets,
                        mask=candid_offsets < num_candids,
                        other=-1,
                    )
                    ...
                    cand_pos_x = tl.load(
                        positions_ptr + candid_ids, mask=valid_mask, other=0.0
                    )
                    cand_pos_y = tl.load(
                        positions_ptr + N + candid_ids, mask=valid_mask, other=0.0
                    )
                    cand_pos_z = tl.load(
                        positions_ptr + 2 * N + candid_ids, mask=valid_mask, other=0.0
                    )
                    ...
                    if only_count == False:
                        ...
                        store_mask = within_cutoff & (
                            nbr_count_shift < max_num_neighbors
                        )
...
def compute_neighbor_list_triton(
    positions,
    lattice_lengths,
    cell_list,
    cutoff,
    ...
    if only_count == False:
        neighbor_list = torch.full(
            (N, max_num_neighbors), -1, dtype=torch.int32, device=positions.device
        )
    else:
        neighbor_list = None
...
class NeighborState:
    ...
    def __post_init__(self):
        ...
        nbr_inds = generate_neighbor_list(
            self.coords,
            self.lattice_vecs,
        ...
        if self.lattice_vecs != None:
            ...
            self.lattice_lengths = (
                self.coords.max(dim=1)[0] - self.coords.min(dim=1)[0] + 1.0
            )
    ...
    def update(self, new_coords):
        ...
        # if True:
        if neigh_state_dist_check(
            self.coords, new_coords, self.lattice_lengths, self.buffer
        ):
            nbr_inds = generate_neighbor_list(
                new_coords,
                self.lattice_vecs,
    ...
    def calculate_displacement(self, coords):
        if self.is_dense:
            ...
            return calculate_displacement_sparse(
                coords, self.nbr_inds, self.lattice_lengths
            )
...
@torch.compile
def calculate_displacement_sparse(
    coords: Tensor, nbr_ids: Tensor, lattice_lengths: Tensor
):
    """
...
@torch.compile
def calculate_mask(
    coords: Tensor, nbr_ids: Tensor, lattice_lengths: Tensor, cutoff: float
):
...
def self_mask(idx):
    ...
    """
    self_mask = idx == torch.reshape(
        torch.arange(idx.shape[0], dtype=torch.int32, device=idx.device),
        (idx.shape[0], 1),
    )
    return torch.where(self_mask, DUMMY_IND, idx)


def unflatten_cell_buffer(arr: Tensor, cells_per_side: Tensor, dim: int):
...
# investigate
def count_flattened_cell_sizes(
    cell_inds: Tensor, lattice_lengths: Tensor, cell_size: float
):
    """
    ...
    [cell_size_per_dim, cells_per_side, cell_count] = calculate_cell_dimensions(
        lattice_lengths, cell_size
    )
    ...
    particle_flat_cell_inds = torch.sum(
        cell_inds * offset_vals[:, None], dtype=torch.int32, dim=0
    )
    flat_cell_sizes = torch.zeros(
        cell_count, dtype=torch.int32, device=cell_inds.device
    )
    # reduce the counts
    flat_cell_sizes = flat_cell_sizes.index_add_(
        0, particle_flat_cell_inds, torch.ones_like(particle_flat_cell_inds)
    )
    return flat_cell_sizes


def populate_cells(
    cell_inds: Tensor, cells_per_side: Tensor, cell_count: int, max_cell_capacity: int
):
    """
    ...
    cells = DUMMY_IND + torch.zeros(
        (cell_count * max_cell_capacity,), dtype=torch.int32, device=device
    )
...
def generate_candidates(cells: Tensor, N: int, all_shifts: list):
    ...
    neighbor_idx = DUMMY_IND + torch.zeros(
        (N + 1, num_candids), dtype=torch.int32, device=cells.device
    )
...
def create_dense_neighbor_list(
    ...
    out_idx = DUMMY_IND + torch.zeros(
        candid_ids.shape, dtype=torch.int32, device=coords.device
    )
...
def generate_neighbor_list(
    coords: Tensor,
    lattice_vectors: Union[Tensor, None],
    cutoff: float,
    is_dense: bool = True,
    use_triton: bool = False,
):
    ...
    if not use_triton:
        ...
        if is_dense:
            return create_dense_neighbor_list(
                coords, lattice_lengths, candid_ids, cutoff
            )
        else:
            return create_sparse_neighbor_list(
                coords, lattice_lengths, candid_ids, cutoff
            )
    else:
        max_nbr_limit = int(0.1 * cutoff**3 * 4 / 3 * torch.pi * 1.5)
        candid_ids, nbr_counts = compute_neighbor_list_triton(
            coords,
            lattice_lengths,
            cells,
            cutoff,
