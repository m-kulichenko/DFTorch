...
def get_nl(structure, dftorch_params):
    ...
    nbr_state = NeighborState(positions, structure.lattice_vecs, None, dftorch_params['cutoff'], is_dense=True, buffer=2.0, use_triton=False)
    disps, dists, nl = calculate_dist_dips(positions, nbr_state, dftorch_params['graph_cutoff'])
...
def get_ch(nparts, structure, dftorch_params, nl, MAX_DEG, NJUMPS):

    

    parts  = graph_partition(structure, structure, graphNL, "SpectralClustering", nparts, positions.T.cpu().numpy(), verb=False)
    partsCoreHalo = []
    numCores = []
    print("\nCore and halos indices for every part:")
    for i in range(nparts):
        coreHalo, nc, nh = get_coreHaloIndices(parts[i], graphNL, NJUMPS)
...
def gather_1d_to_rank0(x: torch.Tensor, device, src: int = 0) -> torch.Tensor:
    ...
    sizes = torch.tensor([int(t.item()) for t in n_all], dtype=torch.int64, device=device)
    ...
    # 3) gather padded
    gathered = [torch.empty((max_n,), dtype=x.dtype, device=device) for _ in range(world)]
...
def get_ij(TYPE, TYPE_all, nnType, nrnnlist, const):
    ...
    neighbor_mask = torch.arange(max_neighbors, device=nnType.device).unsqueeze(0) < nrnnlist
    neighbor_J = nnType[neighbor_mask]
    neighbor_I = torch.repeat_interleave(torch.arange(nrnnlist.squeeze(-1).shape[0], device=nnType.device), nrnnlist.squeeze(-1))
    del neighbor_mask
...
def get_subsy_on_rank(structure, dftorch_params, q, ch, core_size, CoulPot, device):
    ...
    ch_structure = Structure(None, structure.LBox, structure.const, charge=0, Te=structure.Te, device=device,
                             species=structure.TYPE[ch].unsqueeze(0), coordinates=structure.coordinates.T[ch].unsqueeze(0), ignore_spin=True)
    
    ch_structure.core_ao_slice = ch_structure.n_orbitals_per_atom[:core_size].sum()
    ch_structure.ch = ch
    ch_structure.core_size = core_size
    ch_structure.KK = -dftorch_params['SCF_ALPHA']*torch.eye(ch_structure.Nats, device=device)  # Initial mixing coefficient for linear mixing
    
    positions = torch.stack((ch_structure.RX, ch_structure.RY, ch_structure.RZ), )
    nbr_state = NeighborState(positions, ch_structure.lattice_vecs, None, dftorch_params['h0_cutoff'], is_dense=True, buffer=0.0, use_triton=False)
    disps_sub, dists_sub, nl_sub = calculate_dist_dips(positions, nbr_state, dftorch_params['h0_cutoff'])
    ...
    nnType = nl_sub[:,1:]
    neighbor_I, neighbor_J, IJ_pair_type, JI_pair_type = get_ij(ch_structure.TYPE, structure.TYPE, nnType, nl_sub[:,0].unsqueeze(1), structure.const)

    ch_structure.H0, ch_structure.dH0, ch_structure.S, ch_structure.dS = H0_and_S_vectorized(
        ch_structure.TYPE, ch_structure.RX, ch_structure.RY, ch_structure.RZ,
        ch_structure.diagonal, ch_structure.H_INDEX_START,
        nnRx, nnRy, nnRz, nnType,
        ch_structure.const, neighbor_I, neighbor_J, IJ_pair_type, JI_pair_type,
        ch_structure.const.R_orb, ch_structure.const.coeffs_tensor,
        verbose=False)
    ...
    ch_structure.atom_ids = torch.repeat_interleave(torch.arange(len(ch_structure.n_orbitals_per_atom), device=device), ch_structure.n_orbitals_per_atom) # Generate atom index for each orbital
    Hubbard_U_gathered = ch_structure.Hubbard_U[ch_structure.atom_ids]
    ch_structure.hindex = torch.cat((ch_structure.H_INDEX_START, ch_structure.H_INDEX_END[-1:]+1), dim=0)

    if ch_structure.e_field.abs().sum() > 0.0:
        Hdipole = torch.diag(-ch_structure.RX[ch_structure.atom_ids] * ch_structure.e_field[0] - ch_structure.RY[ch_structure.atom_ids] * ch_structure.e_field[1] - ch_structure.RZ[ch_structure.atom_ids] * ch_structure.e_field[2])
    ...
    if q is not None:
        Hcoul_diag = Hubbard_U_gathered * q[ch_structure.atom_ids] + CoulPot[ch][ch_structure.atom_ids]
        Hcoul = 0.5 * (Hcoul_diag.unsqueeze(1) * ch_structure.S + ch_structure.S * Hcoul_diag.unsqueeze(0))
    else:
        Hcoul = 0.0
...
def get_energy_on_rank(per_part_data, per_part_D, device):
    ...
    for ch_structure, D in zip(per_part_data, per_part_D):
        # Band energy
        e_band0 = e_band0 + 2*(ch_structure.H0[:ch_structure.core_ao_slice] * D[:ch_structure.core_ao_slice]).sum()
...
def get_forces_on_rank(structure, per_part_data, per_part_D, q_global, dq_p1, device):
    ...
    for ch_structure, D in zip(per_part_data, per_part_D):
        ...
        FScoul.scatter_add_(1, ch_structure.atom_ids.expand(3, -1), dDS_XYZ_row_sum) # sums elements from DS into q based on number of AOs, e.g. x4 p orbs for carbon or x1 for hydrogen
        ...
        Fband0.scatter_add_(1, ch_structure.atom_ids.expand(3, -1), TMP) # sums elements from DS into q based on number of AOs, e.g. x4 p orbs for carbon or x1 for hydrogen
        ...
        FPulay.scatter_add_(1, ch_structure.atom_ids.expand(3, -1), TMP) # sums elements from DS into q based on number of AOs, e.g. x4 p orbs for carbon or x1 for hydrogen

        ### Fdipole = q_i * E
        if structure.e_field.abs().sum() > 0.0:
            Fdipole = q_global[ch_structure.ch].unsqueeze(0) * structure.e_field.view(3, 1)
        else:
            Fdipole = torch.zeros_like(Fband0)

        ### FSdipole
        D0 = torch.diag(ch_structure.D0)
        dotRE = ch_structure.RX * ch_structure.e_field[0] + ch_structure.RY * ch_structure.e_field[1] + ch_structure.RZ * ch_structure.e_field[2]
        ...
        f_tot[:, ch_structure.ch[:ch_structure.core_size]] = \
            FScoul[:, :ch_structure.core_size] + Fband0[:, :ch_structure.core_size] + FPulay[:, :ch_structure.core_size] + Fdipole[:, :ch_structure.core_size] + FSdipole[:, :ch_structure.core_size]

    return f_tot


def kernel_global(structure, positions_global, nbr_inds_global, disps_global, dists_global, CALPHA_global, PME_data_global, dftorch_params,
                  FelTol, K0Res_global, per_part_data, mu0, device):
    if dist.get_rank() == 0:
        vi = torch.zeros(structure.Nats, dftorch_params['KRYLOV_MAXRANK'], device=device)
        fi = torch.zeros(structure.Nats, dftorch_params['KRYLOV_MAXRANK'], device=device)
    else:
        vi = None
        fi = None
    ...
    while (I < dftorch_params['KRYLOV_MAXRANK']) and (Fel > FelTol):
        ...
        for ch_structure in per_part_data:
            ...
            #### calc_dq
            d_Hcoul_diag = ch_structure.Hubbard_U[ch_structure.atom_ids] * v[ch_structure.ch][ch_structure.atom_ids] + d_CoulPot[ch_structure.ch][ch_structure.atom_ids]
            d_Hcoul = 0.5 * (d_Hcoul_diag.unsqueeze(1) * ch_structure.S + ch_structure.S * d_Hcoul_diag.unsqueeze(0))
            ...
            fe = 1./(torch.exp(beta*(ch_structure.e_vals-mu0))+1.0)
            ...
            dq_global[ch_structure.ch[:ch_structure.core_size]] = dq[:ch_structure.core_size]
            ...
            dPdMuAOS = 2.0 * torch.diagonal(dPdMu @ ch_structure.S, offset=0, dim1=-2, dim2=-1)

            dq2 = torch.zeros(ch_structure.Nats, device=device)
            dq2.scatter_add_(0, ch_structure.atom_ids, dPdMuAOS)

            dqdmu_global[ch_structure.ch[:ch_structure.core_size]] = dq2[:ch_structure.core_size]
        ...
        for ch_structure in per_part_data:
            dr[ch_structure.ch[:ch_structure.core_size]] = dq_global[ch_structure.ch[:ch_structure.core_size]] - v[ch_structure.ch[:ch_structure.core_size]]
            dr[ch_structure.ch[:ch_structure.core_size]] = ch_structure.KK[:ch_structure.core_size, :ch_structure.core_size] @ dr[ch_structure.ch[:ch_structure.core_size]]
