...
from sedacs.graph import add_graphs, collect_graph_from_rho, adaptive_halo_expansion, symmetrize_graph

from . import get_energy_on_rank, \
    gather_1d_to_rank0, kernel_global, get_energy_on_rank, get_forces_on_rank, get_subsy_on_rank, \
    get_evals_dvals, calc_q_on_rank

def scf(structure, dftorch_params, fullGraph, ch, core_size, nbr_state, disps_global, dists_global, nl, works_per_rank,
        n_jumps, g_thresh, max_deg, device):
    ...
    while (scf_error > dftorch_params['SCF_TOL']) and (scf_iter < dftorch_params['SCF_MAX_ITER']):
        ...
        for i in range(works_per_rank):
            ch[i], core_size[i], nh = get_coreHaloIndices(ch[i][:core_size[i]], fullGraph, n_jumps)
        ...
        per_part_data, per_part_D, mu0, K0Res_global, graphOnRank = \
            scf_step(scf_iter, structure, positions_global, nl, disps_global, dists_global, CALPHA_global, PME_data_global, dftorch_params, \
                     q_global, mu0, ch, core_size, works_per_rank, g_thresh, max_deg, graphOnRank, device)
