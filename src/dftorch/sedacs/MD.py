...
from sedacs.graph import add_graphs, graph_diff_and_update, collect_graph_from_rho, adaptive_halo_expansion, symmetrize_graph
    ...
    def run(self, structure, dftorch_params, num_steps, dt,
        ...
        for md_step in range(num_steps):
            for i in range(works_per_rank):
                ch[i], core_size[i], nh = get_coreHaloIndices(ch[i][:core_size[i]], fullGraph, self.n_jumps)
