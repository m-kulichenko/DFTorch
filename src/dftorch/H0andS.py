...
from .SlaterKosterPair import Slater_Koster_Pair_vectorized, Slater_Koster_Pair_SKF_vectorized, Slater_Koster_Pair_SKF_vectorized_batch
...
def H0_and_S_vectorized(
    ...
    H0 = (H0 + H0.transpose(0,1)) * 0.5  # Enforce exact symmetry. Some DFTB files give slightly asymmetric data.
    ...
    S = (S + S.transpose(0,1)) * 0.5  # Enforce exact symmetry. Some DFTB files give slightly asymmetric data.
    dS = dS.reshape(3,HDIM,HDIM)/27.21138625
    dS = (dS - dS.transpose(1,2)) * 0.5  # Enforce exact symmetry.

    if verbose: print("H0_and_S t {:.1f} s\n".format( time.perf_counter()-start_time1 ))
    return H0, dH0, S, dS
...
def H0_and_S_vectorized_batch(
    ...
    if verbose: print('  Do H and S')
    H0, dH0 = Slater_Koster_Pair_SKF_vectorized_batch(batch_size, HDIM, dR_dxyz, L_mskd, M_mskd, N_mskd, L_dxyz, M_dxyz, N_dxyz,
                                            pair_mask_HH, pair_mask_HX, pair_mask_XH, pair_mask_XX,
                                            pair_mask_HY, pair_mask_XY, pair_mask_YH, pair_mask_YX, pair_mask_YY,
                                            dx, idx, IJ_pair_type, JI_pair_type, coeffs_tensor,
                                            neighbor_I, neighbor_J, safe_I, safe_J, valid_pairs, H_INDEX_START, 0)
    
    H0 = H0.reshape(batch_size, HDIM,HDIM)
    H0 = H0 + torch.diag_embed(diagonal)
    H0 = (H0 + H0.transpose(1,2)) * 0.5  # Enforce exact symmetry. Some DFTB files give slightly asymmetric data.
    ...
    #### S PART ###      
    S, dS = Slater_Koster_Pair_SKF_vectorized_batch(batch_size, HDIM, dR_dxyz, L_mskd, M_mskd, N_mskd, L_dxyz, M_dxyz, N_dxyz,
                                            pair_mask_HH, pair_mask_HX, pair_mask_XH, pair_mask_XX,
                                            pair_mask_HY, pair_mask_XY, pair_mask_YH, pair_mask_YX, pair_mask_YY,
                                            dx, idx, IJ_pair_type, JI_pair_type, coeffs_tensor,
                                            neighbor_I, neighbor_J, safe_I, safe_J, valid_pairs, H_INDEX_START, 1)

    S = S.reshape(batch_size, HDIM,HDIM)/27.21138625
    S = S + torch.eye(HDIM, device=S.device)
    S = (S + S.transpose(1,2)) * 0.5  # Enforce exact symmetry. Some DFTB files give slightly asymmetric data.
...
def H0_and_S_vectorized_OLD_FOR_POLY(TYPE, RX, RY, RZ, LBox, Nr_atoms, diagonal, H_INDEX_START, H_INDEX_END, Znuc,
    ...
    pairs_tensor, pairs_list, label_list = ordered_pairs_from_TYPE(TYPE)
    ...
    D0 = atomic_density_matrix(Nr_atoms, H_INDEX_START, H_INDEX_END, HDIM, Znuc)
