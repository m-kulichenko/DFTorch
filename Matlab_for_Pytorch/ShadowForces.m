function [Ftot,Fband0,Fdipole,FPulay,FScoul,FSdipole] = ShadowForces(H,H0,S,D,D0,dHx,dHy,dHz,dSx,dSy,dSz,Efield,U,q,n,CoulPot,Rx,Ry,Rz,Nats,H_INDEX_START,H_INDEX_END)
...
for Ia = 1:Nats % Derivatives Ra
  ...
  for j = 1:Nats % Get the Mulliken charges for all atoms
    ...
    FScoul(1,Ia) = FScoul(1,Ia) - dQLxdR*(U(j)*n(j) + CoulPot(j));
    FScoul(2,Ia) = FScoul(2,Ia) - dQLydR*(U(j)*n(j) + CoulPot(j));
    FScoul(3,Ia) = FScoul(3,Ia) - dQLzdR*(U(j)*n(j) + CoulPot(j));
  end

end
FScoul = 2*FScoul;

Ftot  = Fband0 + Fdipole + FPulay + FScoul + FSdipole;  % Collected total force
